<template>
	<div style="background-color:#f0ebe7">
		<div class="manvideo-company">
    		<h1 class="manvideoguest-font" style="padding-top:3rem">오늘의 주인공을 축복해주세요</h1>
  			<div id="session" v-if="session">
			<div class="d-flex">
				<div id="main-video" style="margin-top:3rem">
					<user-video style="margin-right:5px;" :stream-manager="mainStreamManager"/>
					<on-off ref="on-off" @audioOnOff="audioOnOff" @videoOnOff="videoOnOff"></on-off>
				</div>
				<div id="video-container" style="margin-top:3rem">
					<user-video style="margin-left:5px;" v-for="sub in subscribers" :key="sub.stream.connection.connectionId" :stream-manager="sub" @click.native="updateMainVideoStreamManager(sub)"/>
				</div>
		</div>
    <button class="btn btn-large btn-danger btnOut" @click="outFirstMan(where)">나가기</button>
  </div>
  </div>
  </div>
